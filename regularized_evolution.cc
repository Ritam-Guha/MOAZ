...
namespace automl_zero {
   ...
   RegularizedEvolution::RegularizedEvolution(
         ...
         Generator* generator, Evaluator* evaluator, double feasible_error, Mutator* mutator, std::string animation_output,
         IntegerT evals_till_now)
      ...
      evals_till_now_(evals_till_now),
      num_individuals_(0) {
         best_error_  = std::numeric_limits<double>::infinity();
         first_feasible_error_found_ = -1;
         animation_out_.open(animation_output, std::ofstream::app);
      }
...
//                std::cout << "algorithm inside RE: " << next_algorithm->ToReadable() << std::endl;
...
//      (mutated.get())->SetEffectiveInstructions();
   ...
   double RegularizedEvolution::GetBestError(){
      return best_error_;
   }

   double RegularizedEvolution::GetFirstFeasibleError(){
...
//          std::cout << "best error till now: " << fitness << std::endl;
         best_error_ = fitness;
...
//             std::cout << best_error_ << " " << feasible_error_ << std::endl;
   ...
   void RegularizedEvolution::MaybePrintProgress() {
      ...
      shared_ptr<const Algorithm> pop_best_algorithm;
       animation_out_ << "\n" << num_individuals_ + evals_till_now_ << std::endl;
      PopulationStats(
            &pop_mean, &pop_stdev, &pop_best_algorithm, &pop_best_fitness);
       IntegerT algorithm_complexity = ComputeCostNew(pop_best_algorithm->setup_) + ComputeCostNew(pop_best_algorithm->predict_) + ComputeCostNew(pop_best_algorithm->learn_);
       animation_out_ << pop_best_fitness << "\t" << algorithm_complexity << std::endl;
