...
class Mutator {
 public:
  ...
  std::vector <std::pair<IntegerT, IntegerT>> get_algo_len_limits();
  MutationTypeList allowed_actions_;
 ...
 private:
  ...
  const IntegerT setup_size_min_;
  const IntegerT setup_size_max_;
  const IntegerT predict_size_min_;
  const IntegerT predict_size_max_;
  const IntegerT learn_size_min_;
  const IntegerT learn_size_max_;
