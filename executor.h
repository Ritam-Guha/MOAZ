...
namespace automl_zero {
    ...
    private:
        ...
        bool TrainNoOptImplControl(IntegerT max_steps, std::vector<double>* errors,
                // See `Train` for more details about the following args.
                                   TaskIterator<F>* train_it);
        ...
        double ValidateControl(std::vector<double>* errors);
    ...
    template <FeatureIndexT F>
    Executor<F>::Executor(const Algorithm& algorithm,
                          const Task<F>& dataset,
...
//       std::cout << "Memory after setup: " << std::endl;
//       memory_.Display();
    }
...
//       std::cout << "Memory before execution: " << std::endl;
//       memory_.Display();
        ...
        double current_fitness = kMinFitness;

//       std::cout << "Memory before train: " << std::endl;
//       memory_.Display();
        while (num_remaining > 0) {
            if (!Train( //SK
                    std::min(num_examples_per_epoch, num_remaining),
                    train_errors, &train_it)) {
//                std::cout << "Memory after train: " << std::endl;
//                memory_.Display();
//                std::cout << "algorithm inside execute: ";
//                std::cout << algorithm_.ToReadable() << std::endl;
            ...
            current_fitness = Validate(valid_errors);
...
//       std::cout << std::endl;
    ...
    // next to TrainOptImpl (below).
    template <FeatureIndexT F>
    inline bool Executor<F>::TrainNoOptImplControl(const IntegerT max_steps,
                                                   std::vector<double>* errors,
                                                   TaskIterator<F>* train_it) {
        if (errors != nullptr) {
            errors->reserve(max_steps);
        ...
        classicRLEnv *game;
        if (CONTROL_TASK == CARTPOLE_TASK)
            game = new cartPole();
        else if (CONTROL_TASK == CARTCENTERING_TASK)
            game = new cartCentering();
        else if (CONTROL_TASK == MOUNTAINCAR_TASK)
            game = new mountainCar();
        else if (CONTROL_TASK == ACROBOT_TASK)
            game = new acrobot();
        mt19937 rng(time(0));
        for (IntegerT step = 0; step < max_steps; ++step) {
            rng.seed(step);
            ...
            while (!game->terminal()){
                // Run predict component function for this example.
                //const Vector<F>& features = train_it->GetFeatures();
                vector<double> feat = game->getStateVec(PARTIALLY_OBSERVABLE);


                Eigen::Map<Eigen::Matrix<double, F, 1> > features (feat.data());
                memory_.vector_[kFeaturesVectorAddress] = features;
                ZeroLabelAssigner<F>::Assign(&memory_);
                if (SKIP_INTRONS) {
                    for (const std::shared_ptr<const Instruction>& instruction :
                ...
                if (CONTROL_TASK == ACROBOT_TASK)
                    rewardSum += game->update(0, memory_.scalar_[kPredictionsScalarAddress], rng);//continuous acrobot
                else if (CONTROL_TASK == CARTPOLE_TASK || CONTROL_TASK == CARTCENTERING_TASK)
                    rewardSum += game->update(memory_.scalar_[kPredictionsScalarAddress] <= 0 ? 0 : 2, 0, rng); //discrete bang-bang action in {0,2}
                else if (CONTROL_TASK == MOUNTAINCAR_TASK){//discrete action in {0,1,2}
                    int act = 1;
                    if (memory_.scalar_[kPredictionsScalarAddress] < -0.5)
                        act = 0;
                    else if (memory_.scalar_[kPredictionsScalarAddress] > 0.5)
                        act = 2;
                    rewardSum += game->update(act, 0, rng); //discrete action in {0,1,2}
                }
            }
            if (CONTROL_TASK == ACROBOT_TASK)
                memory_.scalar_[kPredictionsScalarAddress] = rewardSum/-200; //acrobot err scaled to [0,1], lower is better
            else if (CONTROL_TASK == CARTPOLE_TASK)
                memory_.scalar_[kLabelsScalarAddress] = 1-(rewardSum/game->maxStep()); //cartPole err scaled to [0,1], lower is better
            else if (CONTROL_TASK == CARTCENTERING_TASK)
                memory_.scalar_[kPredictionsScalarAddress] = -1 * rewardSum; //cartCentering err scaled to [0,1], lower is better
            else if (CONTROL_TASK == MOUNTAINCAR_TASK)
                memory_.scalar_[kPredictionsScalarAddress] = rewardSum/(-1 * game->maxStep()); //mountainCar err scaled to [0,1], lower is better
            ...
            //// Check whether we should stop early.
            //const Scalar& label = train_it->GetLabel();
            //const Scalar& label = 0;//acrobot
            const Scalar& label = 0;// discrete RL
            const double abs_error = ErrorComputer<F>::Compute(memory_, label);
    ...
    bool Executor<F>::TrainOptImpl(const IntegerT max_steps,
        ...
        //std::cout << algorithm_.ToReadable() << std::endl << std::endl;
    ...
    struct ProbAccuracyLossAccumulator {
        inline static void Accumulate(
            ...
            //std::cout << "logit: " << logit << ", label: " << label << endl;
...
//       std::cout << "Memory before validation: " << std::endl;
//       memory_.Display();
        double loss = 0.0;
        if (errors != nullptr) {
        ...
        for (IntegerT step = 0; step < num_steps; ++step) {
            ...
            if (SKIP_INTRONS) {
                ...
                //std::cout << "prediction before execution: " << memory_.scalar_[kPredictionsScalarAddress] << std::endl;
//                std::cout << algorithm_.ToReadable() << std::endl;
//                std::cout << "Memory before instructions: " << std::endl;
//                memory_.Display();
                for (const std::shared_ptr<const Instruction>& instruction :
                        algorithm_.predict_) {
                    ExecuteInstruction(*instruction, rand_gen_, &memory_);
                }
//                std::cout << "Memory after instructions: " << std::endl;
//                memory_.Display();
                //std::cout << "prediction after execution: " << memory_.scalar_[kPredictionsScalarAddress] << std::endl;
            }
    ...
    template <FeatureIndexT F>
    double Executor<F>::ValidateControl(std::vector<double>* errors) {
        double loss = 0.0;
        if (errors != nullptr) {
            errors->reserve(dataset_.ValidSteps());
        }
        ...
        //TaskIterator<F> valid_it = dataset_.ValidIterator();
        //cout << "task--dataset " << dataset_.index() << endl;
        classicRLEnv *game;
        ...
        if (CONTROL_TASK == CARTPOLE_TASK){
            ...
            game = new acrobot();
        mt19937 rng(time(0));
        double rewardSumSum = 0;
        /**********************************************************************************/
        ofstream out_memtrace;
        if (num_steps == 2) {
            double _width = 1200;
            double _height = 1200;
            int argc = 1;
            ...
            out_memtrace.open("/Users/ritz/Necessity/MSU/Research/Projects/AutoRL_Outputs/NSGA2/Intron_Removal/Output_Files/" + problem_name + "/replay/memory_trace.csv"); //SET_PATH
            cout << "/Users/ritz/Necessity/MSU/Research/Projects/AutoRL_Outputs/NSGA2/Intron_Removal/Output_Files/" + problem_name + "/replay/memory_trace.csv" << endl;
        }
        /**********************************************************************************/
        for (IntegerT step = 0; step < num_steps; ++step) {
            rng.seed(num_steps+step);//unseen initial conditions
            game->reset(rng);
            double rewardSum = 0;
            while (!game->terminal()){
                // Run predict component function for this example.
                ...
                if (out_memtrace.is_open()) {
                    ...
                    if (game->getStep() == 0) {
                        ...
                        out_memtrace << "no1 no2";
                        out_memtrace << std::endl;
                    }
                    // Print memory trace
                    for (int i = 0; i < 4; i++) {
                        out_memtrace << memory_.scalar_[i] << " ";
                    }
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 4; j++) {
                            out_memtrace << memory_.vector_[i][j] << " ";
                        }
                    }
                    // Print velocities
                    out_memtrace << game->getStateVar(2, false) << " " << game->getStateVar(3, false);
                    out_memtrace << std::endl;
                }
                //////////////////////////////////////////////////////////////////
                ...
                if (num_steps == 1) {
                    game->display_function(step, memory_.scalar_[kPredictionsScalarAddress] <= 0 ? 0 : 2, memory_.scalar_[kPredictionsScalarAddress] <= 0 ? 0 : 2);

                    char filename[80];

                    sprintf(filename, "%s_%05d_%03d.tga","/Users/ritz/Necessity/MSU/Research/Projects/AutoRL_Outputs/NSGA2/Intron_Removal/Output_Files/CarCentering/replay/f", (int)step, game->getStep()); //SET_HOME
            ...
            //rewardSumSum += rewardSum/-200;//acrobot

            // continuous reward
            if (CONTROL_TASK == ACROBOT_TASK)
                rewardSumSum += 1 + (rewardSum/200); //acrobot reward scaled to [0,1], higher is better
            else if (CONTROL_TASK == CARTPOLE_TASK)
                rewardSumSum += rewardSum/game->maxStep(); //cartPole reward scaled to [0,1], higher is better
            else if (CONTROL_TASK == CARTCENTERING_TASK)
                rewardSumSum += 1+rewardSum; //cartCentering reward scaled to [0,1], higher is better
            else if (CONTROL_TASK == MOUNTAINCAR_TASK)
                rewardSumSum += 1 + (rewardSum/game->maxStep()); //mountainCar reward scaled to [0,1], higher is better

            // binary reward
            //rewardSumSum += game->getSolved() ? 1 : 0;
            ...
            double sumError;
            if (CONTROL_TASK == ACROBOT_TASK)
                sumError = rewardSum/-200; //acrobot err scaled to [0,1], lower is better
            else if (CONTROL_TASK == CARTPOLE_TASK)
                sumError = 1-(rewardSum/game->maxStep()); //cartPole err scaled to [0,1], lower is better
            else if (CONTROL_TASK == CARTCENTERING_TASK)
                sumError = -1*rewardSum; //cartCentering err scaled to [0,1], lower is better
            else if (CONTROL_TASK == MOUNTAINCAR_TASK)
                sumError = rewardSum/(-1 * game->maxStep()); //mountainCar err scaled to [0,1], lower is better
            const double abs_error = sumError;
            ...
            if (errors != nullptr) {
                errors->push_back(std::abs(abs_error));
            }
        }
        delete game;
        // Convert to fitness.
        //double fitness = 1-(rewardSumSum/max_steps);//acrobot

        double fitness = rewardSumSum/num_steps;//discrete RL
